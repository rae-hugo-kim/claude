...
이 에이전트들은 `.claude/agents/`에 정의되어 있다.
...
그 외 (탐색/조사/구현)?
  → 직접 처리 또는 빌트인 에이전트 사용 (Explore, general-purpose, OMC executor 등)
```
...
- **근거**: 전 프로젝트·전 기간 세션 로그 실측에서 위임 0회 (같은 기간
  ...
  않음). 외부 조사·DB·리팩터링 작업은 빌트인(Explore/general-purpose/WebSearch)과
  직접 MCP 호출이 흡수했다.
- **재도입 트리거**: 특정 MCP 도메인 작업이 반복되며 직접 처리의 마찰이
  관측될 때, 해당 도메인 1종만 우선 복원(전체 매트릭스 부활 금지).
  복원은 git history의 `.claude/agents/` 정의 참조.
