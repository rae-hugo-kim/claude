...
이번 작업이 `.claude/skills/<name>/`를 수정했다면 전역 미러 `~/.claude/skills/<name>/`도 동기화한다. **precedence가 personal > project라 전역이 stale하면 그게 실행된다**(cf. `seed_evolution_policy.md`는 아니고 skill-sync 메모리). 양쪽이 동일해야 한다.
