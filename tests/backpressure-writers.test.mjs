...
// the writers only touch <cwd>/.omc/harness-state, no git involved.
...
const HOOKS = join(dirname(fileURLToPath(import.meta.url)), '..', '.claude', 'hooks', 'harness');
...
const stateDir = (dir) => join(dir, '.omc', 'harness-state');
const statusOf = (dir) => {
...
test('invalidator: creates the state dir and writes UNKNOWN when nothing exists yet', () => {
  // First code edit of a session: no .omc/harness-state yet — the hook must
...
// --- wiring: settings.json registers all three on the lifecycle they assume ----

test('settings.json wires tracker/invalidator/failure-tracker to the right events', () => {
  const settings = JSON.parse(readFileSync(
    join(dirname(fileURLToPath(import.meta.url)), '..', '.claude', 'settings.json'), 'utf-8'));
  const cmds = (group, matcher) => (settings.hooks[group] || [])
    .filter((e) => (e.matcher ?? '') === matcher)
    .flatMap((e) => e.hooks.map((h) => h.command));
  assert.ok(cmds('PostToolUse', 'Bash').some((c) => c.includes('backpressure-tracker.mjs')),
    'PostToolUse Bash must run the PASS tracker');
  assert.ok(cmds('PostToolUse', 'Edit|Write').some((c) => c.includes('backpressure-invalidator.mjs')),
    'PostToolUse Edit|Write must run the invalidator');
  assert.ok(cmds('PostToolUseFailure', 'Bash').some((c) => c.includes('backpressure-failure-tracker.mjs')),
    'PostToolUseFailure Bash must run the failure tracker');
});
