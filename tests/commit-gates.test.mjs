...
const DISPATCHER = join(dirname(fileURLToPath(import.meta.url)), '..', '.claude', 'hooks', 'harness', 'commit-gates.mjs');
...
function setStatus(dir, status) {
  const sd = join(dir, '.omc', 'harness-state');
...
test('non-commit command short-circuits — no gate child is spawned', () => {
  withRepo(CRITICAL, (dir) => {
    ...
    assert.equal(existsSync(join(dir, '.omc', 'harness-state', 'hook-debug.log')), false,
