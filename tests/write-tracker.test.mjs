...
// session_state.cwd, so the real repo's .omc/harness-state is never touched
...
const HARNESS = join(dirname(fileURLToPath(import.meta.url)), '..', '.claude', 'hooks', 'harness');
...
const SETTINGS = join(dirname(fileURLToPath(import.meta.url)), '..', '.claude', 'settings.json');
...
function readLog(cwd) {
  const p = join(cwd, '.omc', 'harness-state', 'read-log.txt');
...
test('settings.json wires the lifecycle this fix depends on', () => {
  // The fix is safe only because context-gate gates the Edit|Write at PreToolUse BEFORE
  // write-tracker logs it at PostToolUse (success-only). A logic test cannot observe the
  // registration, so a regression that dropped write-tracker (or context-gate) from
  // settings.json would pass silently. Assert the wiring directly.
  const cfg = JSON.parse(readFileSync(SETTINGS, 'utf-8'));
  const commandsFor = (event, matcher) =>
    (cfg.hooks?.[event] ?? [])
      .filter((g) => g.matcher === matcher)
      .flatMap((g) => g.hooks.map((h) => h.command))
      .join('\n');
  assert.match(commandsFor('PreToolUse', 'Edit|Write'), /context-gate\.mjs/,
    'context-gate must gate Edit|Write at PreToolUse');
  assert.match(commandsFor('PostToolUse', 'Edit|Write'), /write-tracker\.mjs/,
    'write-tracker must record Edit|Write at PostToolUse');
});
